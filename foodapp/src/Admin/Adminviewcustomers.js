...
function Adminviewcustomers() {
    ...
    useEffect(()=>{
        axios.post(`http://localhost:3900/viewcust`)
        .then((result)=>{
            console.log(result);
    ...
    const dltfn=((_id)=>{
        {
        axios.post(`http://localhost:3900/deletecust/${_id}`)
        .then((result)=>{
            console.log(result);
...
return(
            <tr>
                ...
                <td id='cn2'><button type='button' className="btn btn-danger" onClick={()=>{dltfn(details._id)}}>Delete</button></td>
            </tr>
)
             })
            }
